...
export default function DEV_ChatPage() {
    ...
    useEffect(
        () => {
            if (!activeServer || activeServer === 'undefined') return;
            getServerChannels(activeServer);
        },
        [activeServer, user.token] // user.token eklenerek token değiştiğinde kanalların güncellenmesi sağlandı
    )
            ...
            try {
                const response = await axios.get(`http://localhost:5000/api/users/allServers`, {
                    headers: {
                        ...
                        "Content-Type": "application/json", //"Cache-Control": "no-cache", // 304'ü engellemek için
                        //"Pragma": "no-cache"
                    },
                });

                const serverIds = response.data;
                const serverDetails = await Promise.all(serverIds.map(async (id) => {
                    try {
    ...
    return (<div className="h-screen w-screen bg-gray-900 text-white flex">
            {/* Sidebar */}
                ...
                <div className="mt-6 flex-1 w-full">
                    ...
                    <ul>
                        {servers.map((srv) => (<li
                                ...
                                <span
                                    className={`text-sm font-medium truncate flex-1 ${activeServer === srv._id ? 'text-blue-400' : 'text-gray-200'}`}
                                    title={srv.name}
            ...
            <aside className="w-20 md:w-50 bg-gray-800 flex flex-col items-center py-4 border-r border-gray-700">
                {/*<CreateChannel/>*/}
                <div className="mt-6 flex-1 w-full">
                    <h2 className="text-xs text-gray-400 mb-2 px-4">Kanallar</h2>
                    <ul>
                        {channels.map((channel) => (<li
                                key={channel._id}
                                className="px-4 py-2 hover:bg-gray-700 rounded cursor-pointer flex items-center">
                                 <span
                                     className={`text-sm font-medium truncate flex-1 ${activeChannel === channel._id ? 'text-blue-400' : 'text-gray-200'}`}
                                     title={channel.name}
                                     onClick={() => navigate(`/chats/${activeServer}/${channel._id}`)}>
                                    {channel.name}
                                </span>
                            </li>))}
                    </ul>
            ...
            <main className="flex-1 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                    <ChatArea user={user} activeChannelId={activeChannel}/>
                </div>
            </main>
        </div>);
}